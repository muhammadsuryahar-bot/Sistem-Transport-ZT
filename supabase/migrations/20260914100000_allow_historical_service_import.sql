-- Historical Excel service rows represent work that happened before the system was used.
-- They do not have the system's approval/evidence artifacts, so the normal
-- completion workflow must not be retroactively forced onto this controlled import path.
-- The exemption is intentionally narrow: only service rows explicitly tagged
-- by the importer as "Import histori Excel:" are allowed to complete this way.
...
BEGIN
  IF NEW.status = 'SELESAI'
     ...
     AND COALESCE(NEW.catatan, '') NOT LIKE 'Import histori Excel:%' THEN
...
FOR EACH ROW
EXECUTE FUNCTION public.guard_service_completion();
