...
function chooseServiceSheet(sheets) {
  return sheets.map((sheet) => {
    ...
    if (name === 'data_service') score += 20
    else if (name.includes('data_service')) score += 12
...
async function importServiceHistory(rows, profile, sheetName, onProgress = () => {}) {
  ...
  groups.forEach((group) => {
    ...
    if (existingKeys.has(duplicateKey)) skippedGroups.push(group)
  ...
  onProgress({ completed: 0, total: candidates.length, imported: 0, skipped: skippedGroups.length })

  const processCandidate = async (candidate, candidateIndex) => {
    const { group, vehicle, duplicateKey } = candidate
    const first = group.values[0]
    ...
    const serviceNumber = `IMP-SRV-${Date.now()}-${candidateIndex + 1}`
    ...
    const completed = await supabase.from('service').update({ status: 'SELESAI', selesai_at: new Date().toISOString() }).eq('id', service.data.id).select('id').single()
    ...
    return {
      ...
      km: kilometer > Number(vehicle.kilometer_terakhir || 0)
        ? { kendaraan_id: vehicle.id, nomor_polisi: first.nomor_polisi, tanggal: first.tanggal, kilometer, keterangan: historyNote }
        : null,
    }
  }

  const workerCount = Math.min(IMPORT_CONCURRENCY, Math.max(candidates.length, 1))
  let cursor = 0
  const worker = async () => {
    while (failure) return
    while (true) {
      if (failure) return
      const index = cursor
      cursor += 1
      ...
      try {
        const result = await processCandidate(candidates[index], index)
        results[index] = result
        completedCount += 1
        onProgress({ completed: completedCount, total: candidates.length, imported: completedCount, skipped: skippedGroups.length })
      } catch (error) {
        failure = error
        return
      }
  ...
  results.filter(Boolean).forEach((result) => {
    importedGroups.push(result.group)
    itemCount += result.itemCount
    existingKeys.add(result.duplicateKey)
  })
  ...
  results.filter(Boolean).forEach((result) => {
    if (!result.km) return
    const current = kmUpdates.get(result.km.kendaraan_id)
    if (!current || result.km.kilometer > current.kilometer) kmUpdates.set(result.km.kendaraan_id, result.km)
    kmHistoryRows.push({
      kendaraan_id: result.km.kendaraan_id,
      tanggal: result.km.tanggal,
      kilometer: result.km.kilometer,
      sumber: 'IMPORT_EXCEL_SERVICE',
  ...
  await Promise.all([...kmUpdates.values()].map(async (update) => {
    const result = await supabase.from('kendaraan').update({ kilometer_terakhir: update.kilometer }).eq('id', update.kendaraan_id)
    if (result.error) throw new Error(`Histori service tersimpan tetapi KM master ${update.nomor_polisi} gagal diperbarui: ${result.error.message}`)
    const vehicle = vehicleMap[update.nomor_polisi]
    if (vehicle) vehicle.kilometer_terakhir = update.kilometer
  }))

  if (kmHistoryRows.length) {
    const historyResult = await supabase.from('riwayat_kilometer').insert(kmHistoryRows)
    if (historyResult.error) throw new Error(`KM master sudah diperbarui tetapi riwayat KM gagal dicatat: ${historyResult.error.message}`)
  }
...
export default function ServiceHistoryImportModal({ profile, onDone, onClose }) {
  ...
  const start = async () => {
    ...
    setSaving(true)
    setError('')
    setProgress({ completed: 0, total: workbook.transactions.length })
    setMessage(`Memproses import: 0/${workbook.transactions.length} transaksi...`)
    try {
      const result = await importServiceHistory(workbook.valid, profile, workbook.sheet.name, ({ completed, total }) => {
        setProgress({ completed, total })
        setMessage(`Memproses import: ${completed}/${total} transaksi...`)
      })
      ...
      const report = { context: 'service', ...result, validRows: workbook.valid.length, transactions: result.imported, fileName: file?.name || '', completedAt: new Date().toISOString() }
      sessionStorage.setItem('transport_import_report', JSON.stringify(report))
      onDone?.(report)
    } catch (e) {
      setError(e.message || 'Import histori service gagal.')
      setMessage('Import berhenti karena terjadi error. Data yang sudah berhasil tersimpan sebelum error tetap ada dan tidak diulang pada import berikutnya.')
    } finally {
      setSaving(false)
    }
  }
      ...
      <section className="dpt-modal service-import-modal">
        ...
        {workbook && <>
          ...
          <div className="service-import-note">
            ...
            <span>Import memproses sampai {IMPORT_CONCURRENCY} transaksi secara bersamaan agar ratusan baris tidak menunggu satu per satu.</span>
            ...
            <span>KM yang lebih baru memperbarui Master Kendaraan dan dicatat ke Riwayat Kilometer.</span>
          </div>
          <div className="service-import-map">
            <div><b>No. Polisi</b><span>→</span><span>Kendaraan</span></div><div><b>Tanggal</b><span>→</span><span>Tanggal Service</span></div><div><b>Jenis Pekerjan</b><span>→</span><span>Jenis Service</span></div><div><b>Uraian</b><span>→</span><span>Item Service</span></div><div><b>Nilai DPP</b><span>→</span><span>DPP</span></div><div><b>PPn</b><span>→</span><span>PPN</span></div><div><b>Total</b><span>→</span><span>Total & Biaya Aktual</span></div><div><b>KM</b><span>→</span><span>KM Service + Master Kendaraan + Riwayat KM</span></div><div><b>Nama Bengkel</b><span>→</span><span>Bengkel</span></div><div><b>Keterangan</b><span>→</span><span>Catatan Item</span></div>
          </div>
          <div className="dpt-preview">
            <div className="dpt-sheet-title"><b>Preview Sumber: {workbook.sheet.name}</b><span>{workbook.dataRows.length} baris sumber</span></div>
            <div className="dpt-preview-wrap"><table><thead><tr>{workbook.header.row.map((header, index) => <th key={`${header}-${index}`}>{header || `Kolom ${index + 1}`}</th>)}</tr></thead><tbody>{workbook.dataRows.map((row) => <tr key={row.excelRow}>{workbook.header.row.map((header, index) => <td key={index}>{clean(row.values[index]) || '-'}</td>)}</tr>)}</tbody></table></div>
          </div>
        </>}
