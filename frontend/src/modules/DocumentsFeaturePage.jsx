...
export default function DocumentsFeaturePage({profile}){
 ...
 const monitorRows=useMemo(()=>vehicles.map((v,index)=>{
  ...
  return {no:Number(meta?.source_no)||index+1,merk:meta?.merk||v.merk||'-',tipe:meta?.type||v.tipe||'-',nomor_polisi:meta?.nomor_polisi||v.nomor_polisi||'-',tahun:meta?.tahun||v.tahun||'-',nomor_rangka:meta?.nomor_rangka||v.nomor_rangka||'-',stnk:byType.STNK?.tanggal_jatuh_tempo||null,foto_stnk_path:v.foto_stnk_path||null,kir:byType.KIR?.tanggal_jatuh_tempo||null,lima_tahun:byType['5_TAHUNAN']?.tanggal_jatuh_tempo||null,pemilik:meta?.pemilik||v.pemilik||'-'}
 }),[vehicles,docs])

 const reminders=useMemo(()=>docs.map(d=>({doc:d,vehicle:vehicleMap[d.kendaraan_id],days:daysLeft(d.tanggal_jatuh_tempo)})).filter(x=>x.days!==null&&x.days<=30).sort((a,b)=>a.days-b.days),[docs,vehicleMap])
 const summary={total:docs.length,expired:docs.filter(d=>statusOf(d.tanggal_jatuh_tempo)==='EXPIRED').length,soon:docs.filter(d=>statusOf(d.tanggal_jatuh_tempo)==='SEGERA').length}
 ...
 return <div className="x-page">
  ...
  {reminders.length>0&&<div className="x-alert warning"><b>Pengingat dokumen:</b> {summary.expired} sudah lewat, {summary.soon} akan jatuh tempo dalam 30 hari. {reminders.slice(0,4).map(x=><span key={x.doc.id}> • {x.vehicle?.nomor_polisi||'-'} {x.doc.jenis_dokumen} ({x.days<0?`lewat ${Math.abs(x.days)} hari`:`${x.days} hari lagi`})</span>)}</div>}
