...
export default function ServiceFeaturePage({ profile }) {
  ...
  const approvalRequirement = s => Number(s.biaya_aktual ?? s.estimasi_biaya ?? 0) > 5000000 ? 'DIREKTUR' : 'ATASAN_TRANSPORT'
  ...
  const requiredApprovalCount = s => additionalApprovalRequired(s) ? 2 : 1
  ...
  const requestApproval = s => {
    ...
    setApprovalModal({ service: s, required, additional: additionalApprovalRequired(s) || (Number(s.biaya_aktual ?? 0) > 5000000 && hasInitialApproval(s)) })
    setApprovalNote('')
  }
  ...
  const confirmApproval = async approved => {
    ...
    try {
      ...
      const next = Math.max(0, ...approvalList(s).map(a => Number(a.urutan) || 0)) + 1
      ...
      if (e2) throw e2
      setSuccess(approved ? `Approval ke-${next} service disetujui oleh ${required === 'DIREKTUR' ? 'Direktur' : 'Atasan Transport'}.` : 'Service ditolak.')
  ...
  const updateActual = async () => {
    ...
    setSaving(true); clearMessages()
    const { error: e } = await supabase.from('service').update({ biaya_aktual: value, status: value > 5000000 && approvedCount(selected) === 0 ? 'MENUNGGU_APPROVAL' : selected.status }).eq('id', selected.id)
    setSaving(false)
    if (e) setError(e.message)
    else { setSelected({ ...selected, biaya_aktual: value }); setSuccess('Biaya aktual diperbarui. Sistem akan meminta approval tambahan bila melewati batas/estimasi.'); await load() }
  }
  ...
  const finish = async s => {
    ...
    if (actual > 5000000 && !hasDirectorApproval(s)) { setError('Biaya aktual di atas Rp5.000.000 memerlukan approval Direktur.'); return }
    if (estimate > 5000000 && count < 1) { setError('Service di atas Rp5.000.000 belum memiliki approval.'); return }
    if (actual > estimate && count < 2) { setError('Biaya aktual melebihi estimasi/approval awal. Approval tambahan wajib diberikan sebelum service selesai.'); return }
    setSaving(true)
    try {
