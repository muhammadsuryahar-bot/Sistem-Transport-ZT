...
const emptyService = { permintaan_service_id: '', kendaraan_id: '', tanggal_service: new Date().toISOString().slice(0, 10), kilometer: '', bengkel: '', jenis_service: 'SERVICE', keluhan: '', estimasi_biaya: '', biaya_aktual: '', nilai_dpp: '', ppn: '', total: '', catatan: '' }
const emptyItem = { service_id: '', nama_item: '', kategori: 'SPAREPART', jumlah: '1', satuan: 'pcs', harga_satuan: '', keterangan: '' }
const emptyProof = { service_id: '', jenis_bukti: 'BON_INVOICE', keterangan: '' }
...
export default function ServiceFeaturePage({ profile }) {
  ...
  const serviceExcelTopScrollRef = useRef(null)
  ...
  useEffect(() => {
    ...
    const handleServiceStateUpdated = event => {
      ...
      if (detail.kind === 'service' && detail.service) {
        setServices(current => {
          const exists = current.some(row => row.id === detail.service.id)
          return exists ? current.map(row => row.id === detail.service.id ? { ...row, ...detail.service } : row) : [detail.service, ...current]
        })
        if (detail.request?.id) setRequests(current => current.map(row => row.id === detail.request.id ? { ...row, ...detail.request } : row))
      }
      if (detail.kind === 'proof' && detail.proof) setProofs(current => current.some(row => row.id === detail.proof.id) ? current : [detail.proof, ...current])
      if (detail.kind === 'approval' && detail.approval) setApprovals(current => current.some(row => row.id === detail.approval.id) ? current : [detail.approval, ...current])
    }
    window.addEventListener('transport:data-imported', handleImported)
    window.addEventListener('transport:service-state-updated', handleServiceStateUpdated)
    return () => {
  ...
  const filteredServices = useMemo(() => { const q = serviceSearch.trim().toLowerCase(); return services.filter(s => { const v = vehicleMap[s.kendaraan_id]; const hay = [s.nomor_service, s.bengkel, s.keluhan, v?.nomor_polisi, v?.merk, v?.tipe].filter(Boolean).join(' ').toLowerCase(); return (!q || hay.includes(q)) && (serviceVehicleFilter === 'SEMUA' || String(s.kendaraan_id) === String(serviceVehicleFilter)) }) }, [services, serviceSearch, serviceVehicleFilter, vehicleMap])
  const refreshServiceExcelScroll = () => {
    const wrap = serviceExcelWrapRef.current
    const table = wrap?.querySelector('.x-table')
    if (!wrap || !table) return
    setServiceExcelScrollWidth(Math.max(table.scrollWidth, wrap.clientWidth))
  }
  const syncServiceExcelFromTop = event => {
    if (serviceExcelWrapRef.current) serviceExcelWrapRef.current.scrollLeft = event.currentTarget.scrollLeft
  }
  const syncServiceExcelFromTable = event => {
    if (serviceExcelTopScrollRef.current) serviceExcelTopScrollRef.current.scrollLeft = event.currentTarget.scrollLeft
  }
  useEffect(() => {
    refreshServiceExcelScroll()
    const wrap = serviceExcelWrapRef.current
    const table = wrap?.querySelector('.x-table')
    if (!wrap || !table) return undefined
    const observer = typeof ResizeObserver !== 'undefined' ? new ResizeObserver(refreshServiceExcelScroll) : null
    observer?.observe(wrap)
    observer?.observe(table)
    window.addEventListener('resize', refreshServiceExcelScroll)
    return () => {
      observer?.disconnect()
  ...
  const saveService = async e => {
    ...
    try {
      if (!form.kendaraan_id) throw new Error('Kendaraan wajib dipilih.')
      if (form.estimasi_biaya === '' || Number(form.estimasi_biaya) < 0) throw new Error('Estimasi biaya wajib diisi.')
      const estimate = Number(form.estimasi_biaya)
      const actual = form.biaya_aktual === '' ? null : Number(form.biaya_aktual)
      if (actual !== null && (!Number.isFinite(actual) || actual < 0)) throw new Error('Biaya aktual tidak valid.')
      const dpp = Number(form.nilai_dpp || estimate), ppn = Number(form.ppn || 0), total = Number(form.total || (dpp + ppn))
      if (![dpp, ppn, total].every(Number.isFinite) || dpp < 0 || ppn < 0 || total < 0) throw new Error('DPP/PPN/Total tidak valid.')
      const needsApproval = Math.max(estimate, actual ?? 0) > 5000000 || (actual !== null && actual > estimate)
      const previous = editingServiceId ? services.find(row => row.id === editingServiceId) : null
      const payload = {
        permintaan_service_id: form.permintaan_service_id ? Number(form.permintaan_service_id) : null,
        kendaraan_id: Number(form.kendaraan_id), tanggal_service: form.tanggal_service, kilometer: Number(form.kilometer || 0),
        bengkel: form.bengkel.trim() || null, jenis_service: form.jenis_service, keluhan: form.keluhan.trim() || null,
        estimasi_biaya: estimate, biaya_aktual: actual, status: needsApproval ? 'MENUNGGU_APPROVAL' : (editingServiceId ? (previous?.status || 'DALAM_PENGERJAAN') : 'DALAM_PENGERJAAN'),
        diproses_oleh: profile.id, nilai_dpp: dpp, ppn, total, catatan: form.catatan.trim() || null,
      }
  ...
  const saveItem = async e => {
    ...
    if (!Number.isFinite(harga) || harga < 0) return setError('Harga satuan tidak valid.')
    const payload = { service_id: Number(itemForm.service_id), nama_item: itemForm.nama_item.trim(), kategori: itemForm.kategori.trim() || null, jumlah, satuan: itemForm.satuan, harga_satuan: harga, subtotal: jumlah * harga, keterangan: itemForm.keterangan.trim() || null }
    setSaving(true)
    try {
      const result = editingItemId
        ? await supabase.from('service_item').update(payload).eq('id', editingItemId).select('*').single()
        : await supabase.from('service_item').insert(payload).select('*').single()
      if (result.error) throw result.error
  ...
  const saveProof = async e => {
    ...
    let path = null
    try {
      if (!proofForm.service_id) throw new Error('Service wajib dipilih.')
      let filePath = editingProofId ? (proofs.find(p => p.id === editingProofId)?.file_path || null) : null
      if (file) {
        path = await upload('service-bukti', file, 'service/' + proofForm.service_id)
        filePath = path
      }
      if (!filePath) throw new Error('File bukti wajib dipilih.')
      const payload = { service_id: Number(proofForm.service_id), jenis_bukti: proofForm.jenis_bukti, nama_file: file?.name || proofs.find(p => p.id === editingProofId)?.nama_file || 'Bukti Service', file_path: filePath, keterangan: proofForm.keterangan.trim() || null }
      const result = editingProofId
        ? await supabase.from('service_bukti').update(payload).eq('id', editingProofId).select('*').single()
        : await supabase.from('service_bukti').insert({ ...payload, uploaded_by: profile.id }).select('*').single()
      if (result.error) throw result.error
      if (editingProofId && file && proofs.find(p => p.id === editingProofId)?.file_path) await supabase.storage.from('service-bukti').remove([proofs.find(p => p.id === editingProofId).file_path])
      setProofs(current => editingProofId ? current.map(row => row.id === result.data.id ? result.data : row) : [result.data, ...current])
  ...
  return <div className="x-page"><Header eyebrow="SERVICE & MAINTENANCE" title="Service & Perbaikan" text="Kelola pekerjaan, approval, rincian item, bukti, ban, aki, dan kilometer." action={<button className="x-btn secondary" onClick={load}>↻ Refresh</button>} />{error && <Alert type="error">{error}</Alert>}{success && <Alert>{success}</Alert>}<div className="x-tabs">{tabs.map(([v, l]) => <button key={v} className={tab === v ? 'active' : ''} onClick={() => { clearMessages(); setTab(v) }}>{l}</button>)}</div>{tab === 'excel' && <section className={`x-card service-excel-card ${serviceExcelFullscreen ? 'service-excel-card-fullscreen' : ''}`}>
    ...
    <div className="service-excel-wide-body">
      <div className="service-excel-scroll-label"><span>Geser kolom secara horizontal</span><small>Baris tetap di tempat, scrollbar tersedia di bagian atas</small></div>
      <div className="service-excel-top-scroll" ref={serviceExcelTopScrollRef} onScroll={syncServiceExcelFromTop} aria-label="Geser tabel secara horizontal">
        <div style={{ width: `${serviceExcelScrollWidth}px`, height: '1px' }} />
      </div>
      <div className="x-table-wrap" ref={serviceExcelWrapRef} onScroll={syncServiceExcelFromTable}>
        <table className="x-table">
          <thead><tr><th>No</th><th>Merk</th><th>Type</th><th>Jenis</th><th>Tahun</th><th>No. Polisi</th><th>Driver/PIC</th><th>Bulan</th><th>Tanggal</th><th>Jenis Pekerjan</th><th>Uraian</th><th>Qty</th><th>Sat</th><th>Harga Satuan (Rp)</th><th>Nilai DPP</th><th>PPn</th><th>Total</th><th>KM</th><th>Nama Bengkel</th><th>Keterangan</th></tr></thead>
          <tbody>{serviceExcelRows.length ? serviceExcelRows.map(row => <tr key={`${row.no}-${row.nomor_polisi}-${row.tanggal}-${row.uraian}`}><td>{row.no}</td><td>{row.merk}</td><td>{row.type}</td><td>{row.jenis}</td><td>{row.tahun}</td><td>{row.nomor_polisi}</td><td>{row.driver}</td><td>{row.bulan}</td><td>{row.tanggal}</td><td>{row.jenis_pekerjaan}</td><td>{row.uraian}</td><td>{row.qty}</td><td>{row.satuan}</td><td>{money(row.harga_satuan)}</td><td>{money(row.nilai_dpp)}</td><td>{row.ppn === '-' ? '-' : money(row.ppn)}</td><td>{money(row.total)}</td><td>{row.kilometer}</td><td>{row.bengkel}</td><td>{row.keterangan}</td></tr>) : <tr><td colSpan="20"><Empty text="Belum ada riwayat service." /></td></tr>}</tbody>
        </table>
      </div>
    </div>
  </section>}{tab === 'pekerjaan' && <><section className="x-card">{canProcess && <><div className="x-card-title"><h3>{editingServiceId ? 'Edit Data Service' : 'Tambah Data Service'}</h3><p>Pengajuan opsional. Admin dapat mencatat service langsung tanpa kembali ke Excel.</p></div><form className="x-grid" onSubmit={saveService}><label>Pengajuan (opsional)<select value={form.permintaan_service_id} onChange={e => { const r = requests.find(x => String(x.id) === e.target.value); setForm(f => ({ ...f, permintaan_service_id: e.target.value, kendaraan_id: r?.kendaraan_id || f.kendaraan_id, kilometer: r?.kilometer_pengajuan ?? vehicleMap[r?.kendaraan_id]?.kilometer_terakhir ?? f.kilometer, jenis_service: r?.jenis_permintaan || f.jenis_service || 'SERVICE', keluhan: r?.keluhan || f.keluhan })) }}><option value="">Tanpa Pengajuan</option>{pending.map(r => <option key={r.id} value={r.id}>{r.nomor_pengajuan || `#${r.id}`} — {TYPES[r.jenis_permintaan] || r.jenis_permintaan}</option>)}</select></label><label>Kendaraan<select value={form.kendaraan_id} onChange={e => setForm(f => ({ ...f, kendaraan_id: e.target.value }))}><option value="">Pilih</option>{vehicles.map(v => <option key={v.id} value={v.id}>{v.nomor_polisi} — {v.merk} {v.tipe || ''}</option>)}</select></label><label>Tanggal<input type="date" value={form.tanggal_service} onChange={e => setForm(f => ({ ...f, tanggal_service: e.target.value }))} /></label><label>KM<input type="number" min="0" value={form.kilometer} onChange={e => setForm(f => ({ ...f, kilometer: e.target.value }))} /></label><label>Bengkel<input value={form.bengkel} onChange={e => setForm(f => ({ ...f, bengkel: e.target.value }))} /></label><label>Jenis<select value={form.jenis_service} onChange={e => setForm(f => ({ ...f, jenis_service: e.target.value }))}>{Object.entries(TYPES).map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></label><label>Estimasi Biaya<input type="number" min="0" value={form.estimasi_biaya} onChange={e => setForm(f => ({ ...f, estimasi_biaya: e.target.value }))} /></label><label>Biaya Aktual<input type="number" min="0" value={form.biaya_aktual} onChange={e => setForm(f => ({ ...f, biaya_aktual: e.target.value }))} /></label><label className="full">Keluhan<input value={form.keluhan} onChange={e => setForm(f => ({ ...f, keluhan: e.target.value }))} /></label><label className="full">Catatan<textarea value={form.catatan} onChange={e => setForm(f => ({ ...f, catatan: e.target.value }))} /></label><div className="full x-actions"><button type="button" className="x-btn secondary" onClick={() => { setEditingServiceId(null); setForm({ ...emptyService, tanggal_service: new Date().toISOString().slice(0, 10) }) }} disabled={saving}>Bersihkan</button><button className="x-btn primary" disabled={saving}>{saving ? 'Menyimpan…' : editingServiceId ? 'Perbarui Service' : 'Simpan Service'}</button></div></form></>}</section><section className="x-card"><div className="x-card-title"><div><h3>Daftar Service</h3><p>Cari berdasarkan BM/nomor polisi, merk, type, bengkel, atau keluhan. Aksi detail, edit, dan hapus dibuat ringkas agar tabel tetap bersih.</p></div></div><div className="x-toolbar-inline"><input value={serviceSearch} onChange={e => setServiceSearch(e.target.value)} placeholder="Cari BM, merk, type, bengkel..." /><select value={serviceVehicleFilter} onChange={e => setServiceVehicleFilter(e.target.value)}><option value="SEMUA">Semua kendaraan</option>{vehicles.map(v => <option key={v.id} value={v.id}>{v.nomor_polisi} — {v.merk}</option>)}</select></div>{loading ? <Empty text="Memuat…" /> : filteredServices.length === 0 ? <Empty text="Belum ada service yang cocok." /> : <div className="x-table-wrap"><table className="x-table"><thead><tr><th>No</th><th>Merk</th><th>Type</th><th>No. Polisi</th><th>Jenis</th><th>Tanggal</th><th>KM</th><th>Bengkel</th><th>Uraian</th><th>Total</th><th>Aksi</th></tr></thead><tbody>{filteredServices.map((s, index) => { const vehicle=vehicleMap[s.kendaraan_id]; const lineItems=items.filter(i=>i.service_id===s.id); const uraian=lineItems.length?lineItems.map(i=>i.nama_item).filter(Boolean).join(' • '):(s.keluhan||'-'); const total=Number(s.total ?? s.biaya_aktual ?? s.estimasi_biaya ?? 0); return <tr key={s.id}><td>{index+1}</td><td><b>{vehicle?.merk||'-'}</b></td><td>{vehicle?.tipe||'-'}</td><td><b>{vehicle?.nomor_polisi||'-'}</b></td><td>{TYPES[s.jenis_service]||s.jenis_service||'-'}</td><td>{dateText(s.tanggal_service)}</td><td>{Number(s.kilometer||0).toLocaleString('id-ID')} km</td><td>{s.bengkel||'-'}</td><td>{uraian}</td><td><b>{money(total)}</b></td><td className="x-action-compact"><button className="x-link" onClick={()=>openDetail(s)}>Detail</button>{canProcess&&<button className="x-link" onClick={()=>openServiceEdit(s)}>Edit</button>}{canProcess&&<button className="x-link danger" onClick={()=>deleteService(s)} disabled={saving}>Hapus</button>}</td></tr>})}</tbody></table></div>}</section></>}{tab === 'item' && <><section className="x-card">{canProcess && <><div className="x-card-title"><h3>{editingItemId ? 'Edit Item Service' : 'Tambah Item Service'}</h3><p>Catat jasa, suku cadang, oli, ban, atau aki dengan jumlah dan harga satuan.</p></div><form className="x-grid" onSubmit={saveItem}><label>Service<select value={itemForm.service_id} onChange={e => setItemForm(f => ({ ...f, service_id: e.target.value }))}><option value="">Pilih</option>{services.map(s => <option key={s.id} value={s.id}>{s.nomor_service || `#${s.id}`}</option>)}</select></label><label>Nama Item<input value={itemForm.nama_item} onChange={e => setItemForm(f => ({ ...f, nama_item: e.target.value }))} /></label><label>Kategori<select value={itemForm.kategori} onChange={e => setItemForm(f => ({ ...f, kategori: e.target.value }))}><option value="JASA">Jasa</option><option value="SPAREPART">Sparepart</option><option value="OLI">Oli</option><option value="BAN">Ban</option><option value="AKI">Aki</option><option value="LAINNYA">Lainnya</option></select></label><label>Jumlah<input type="number" min="0" step="0.01" value={itemForm.jumlah} onChange={e => setItemForm(f => ({ ...f, jumlah: e.target.value }))} /></label><label>Satuan<input value={itemForm.satuan} onChange={e => setItemForm(f => ({ ...f, satuan: e.target.value }))} /></label><label>Harga Satuan<input type="number" min="0" value={itemForm.harga_satuan} onChange={e => setItemForm(f => ({ ...f, harga_satuan: e.target.value }))} /></label><label className="full">Keterangan<input value={itemForm.keterangan} onChange={e => setItemForm(f => ({ ...f, keterangan: e.target.value }))} /></label><div className="full x-actions"><button type="button" className="x-btn secondary" onClick={() => { setEditingItemId(null); setItemForm(emptyItem) }}>Bersihkan</button><button className="x-btn primary" disabled={saving}>{editingItemId ? 'Perbarui Item' : 'Simpan Item'}</button></div></form></>}</section><section className="x-card"><div className="x-table-wrap"><table className="x-table"><thead><tr><th>Service</th><th>Item</th><th>Kategori</th><th>Jumlah</th><th>Harga</th><th>Subtotal</th><th>Aksi</th></tr></thead><tbody>{items.length ? items.map(i => <tr key={i.id}><td>#{i.service_id}</td><td>{i.nama_item}</td><td>{i.kategori || '-'}</td><td>{i.jumlah} {i.satuan}</td><td>{money(i.harga_satuan)}</td><td>{money(i.subtotal)}</td><td className="x-action-compact"><button className="x-link" onClick={() => setSelectedItem(i)}>Detail</button>{canProcess && <button className="x-link" onClick={() => editItem(i)}>Edit</button>}{canProcess && <button className="x-link danger" onClick={() => deleteItem(i)} disabled={saving}>Hapus</button>}</td></tr>) : <tr><td colSpan="7"><Empty /></td></tr>}</tbody></table></div></section></>}{tab === 'bukti' && <><section className="x-card">{canProcess && <><div className="x-card-title"><h3>{editingProofId ? 'Edit Bukti Service' : 'Upload Bukti Service'}</h3><p>Gunakan invoice/bon atau foto sebagai dokumentasi pekerjaan.</p></div><form className="x-grid" onSubmit={saveProof}><label>Service<select value={proofForm.service_id} onChange={e => setProofForm(f => ({ ...f, service_id: e.target.value }))}><option value="">Pilih</option>{services.map(s => <option key={s.id} value={s.id}>{s.nomor_service || `#${s.id}`}</option>)}</select></label><label>Jenis Bukti<select value={proofForm.jenis_bukti} onChange={e => setProofForm(f => ({ ...f, jenis_bukti: e.target.value }))}><option value="BON_INVOICE">Invoice / Bon</option><option value="FOTO_SEBELUM">Foto Sebelum</option><option value="FOTO_SESUDAH">Foto Sesudah</option><option value="FOTO_KM">Foto KM</option><option value="DOKUMEN_LAINNYA">Dokumen Lainnya</option></select></label><label>File<input type="file" accept=".pdf,image/*" onChange={e => setFile(e.target.files?.[0] || null)} /></label><label className="full">Keterangan<input value={proofForm.keterangan} onChange={e => setProofForm(f => ({ ...f, keterangan: e.target.value }))} /></label><div className="full x-actions"><button type="button" className="x-btn secondary" onClick={() => { setEditingProofId(null); setProofForm(emptyProof); setFile(null) }}>Bersihkan</button><button className="x-btn primary" disabled={saving}>{editingProofId ? 'Perbarui Bukti' : 'Upload Bukti'}</button></div></form></>}</section><section className="x-card">{proofs.length === 0 ? <Empty /> : <div className="x-table-wrap"><table className="x-table"><thead><tr><th>Service</th><th>Jenis</th><th>File</th><th>Aksi</th></tr></thead><tbody>{proofs.map(p => <tr key={p.id}><td>#{p.service_id}</td><td>{p.jenis_bukti}</td><td>{p.nama_file}</td><td className="x-action-compact"><button className="x-link" onClick={async () => { try { const u = await signed('service-bukti', p.file_path); if (u) window.open(u, '_blank', 'noopener,noreferrer') } catch (e) { setError(e.message) } }}>Lihat</button>{canProcess && <button className="x-link" onClick={() => editProof(p)}>Edit</button>}{canProcess && <button className="x-link danger" onClick={() => deleteProof(p)} disabled={saving}>Hapus</button>}</td></tr>)}</tbody></table></div>}</section></>}{(tab === 'ban' || tab === 'aki') && <><section className="x-card">{canProcess && <><div className="x-card-title"><h3>{editingPartId ? 'Edit Riwayat ' : 'Catat Penggantian '}{tab === 'ban' ? 'Ban' : 'Aki / Baterai'}</h3><p>Foto kondisi sebelum menjadi dokumentasi. Saat edit, foto lama tetap digunakan bila tidak diganti.</p></div><form className="x-grid" onSubmit={savePart}><label>Kendaraan<select value={partForm.kendaraan_id} onChange={e => setPartForm(f => ({ ...f, kendaraan_id: e.target.value, _type: tab.toUpperCase() }))}><option value="">Pilih</option>{vehicles.map(v => <option key={v.id} value={v.id}>{v.nomor_polisi} — {v.merk}</option>)}</select></label><label>Tanggal<input type="date" value={partForm.tanggal_penggantian} onChange={e => setPartForm(f => ({ ...f, tanggal_penggantian: e.target.value, _type: tab.toUpperCase() }))} /></label><label>KM<input type="number" min="0" value={partForm.kilometer} onChange={e => setPartForm(f => ({ ...f, kilometer: e.target.value }))} /></label><label>Alasan<input value={partForm.alasan_penggantian} onChange={e => setPartForm(f => ({ ...f, alasan_penggantian: e.target.value }))} /></label><label>Kondisi Sebelum<input value={partForm.kondisi_sebelum} onChange={e => setPartForm(f => ({ ...f, kondisi_sebelum: e.target.value }))} /></label><label>Biaya<input type="number" min="0" value={partForm.biaya} onChange={e => setPartForm(f => ({ ...f, biaya: e.target.value }))} /></label>{tab === 'ban' ? <><label>Jumlah Ban<input type="number" min="1" value={partForm.jumlah_ban || 1} onChange={e => setPartForm(f => ({ ...f, jumlah_ban: e.target.value, _type: 'BAN' }))} /></label><label>Posisi Ban<input value={partForm.posisi_ban || ''} onChange={e => setPartForm(f => ({ ...f, posisi_ban: e.target.value, _type: 'BAN' }))} /></label><label>Merek Ban<input value={partForm.merek_ban || ''} onChange={e => setPartForm(f => ({ ...f, merek_ban: e.target.value, _type: 'BAN' }))} /></label><label>Ukuran<input value={partForm.ukuran_ban || ''} onChange={e => setPartForm(f => ({ ...f, ukuran_ban: e.target.value, _type: 'BAN' }))} /></label></> : <><label>Merek Aki<input value={partForm.merek_aki || ''} onChange={e => setPartForm(f => ({ ...f, merek_aki: e.target.value, _type: 'AKI' }))} /></label><label>Tipe Aki<input value={partForm.tipe_aki || ''} onChange={e => setPartForm(f => ({ ...f, tipe_aki: e.target.value, _type: 'AKI' }))} /></label><label>Nomor Aki<input value={partForm.nomor_aki || ''} onChange={e => setPartForm(f => ({ ...f, nomor_aki: e.target.value, _type: 'AKI' }))} /></label></>}<label className="full">Foto Kondisi Sebelum<input type="file" accept="image/*" onChange={e => setFile(e.target.files?.[0] || null)} /></label><label className="full">Keterangan<input value={partForm.keterangan} onChange={e => setPartForm(f => ({ ...f, keterangan: e.target.value }))} /></label><div className="full x-actions"><button type="button" className="x-btn secondary" onClick={() => { setEditingPartId(null); setFile(null); setPartForm({ ...emptyPart, _type: tab.toUpperCase() }) }}>Bersihkan</button><button className="x-btn primary" disabled={saving}>{editingPartId ? 'Perbarui Riwayat' : 'Simpan Riwayat'}</button></div></form></>}</section><section className="x-card"><div className="x-table-wrap"><table className="x-table"><thead><tr><th>Kendaraan</th><th>Tanggal</th><th>KM</th><th>Alasan</th><th>Biaya</th><th>Aksi</th></tr></thead><tbody>{(tab === 'ban' ? bans : akis).map(r => <tr key={r.id}><td>{vehicleMap[r.kendaraan_id]?.nomor_polisi || '-'}</td><td>{dateText(r.tanggal_penggantian)}</td><td>{Number(r.kilometer || 0).toLocaleString('id-ID')} km</td><td>{r.alasan_penggantian || '-'}</td><td>{money(r.biaya)}</td><td className="x-action-compact"><button className="x-link" onClick={() => setSelectedPart(r)}>Detail</button>{canProcess && <button className="x-link" onClick={() => editPart(r)}>Edit</button>}{canProcess && <button className="x-link danger" onClick={() => deletePart(r)} disabled={saving}>Hapus</button>}</td></tr>)}</tbody></table></div></section></>}{tab === 'km' && <><section className="x-card">{canProcess && <><div className="x-card-title"><h3>{editingKmId ? 'Edit Riwayat Kilometer' : 'Catat Kilometer'}</h3><p>KM dipakai untuk menghitung jarak pemakaian kendaraan.</p></div><form className="x-grid" onSubmit={saveKm}><label>Kendaraan<select value={kmForm.kendaraan_id} onChange={e => setKmForm(f => ({ ...f, kendaraan_id: e.target.value }))}><option value="">Pilih</option>{vehicles.map(v => <option key={v.id} value={v.id}>{v.nomor_polisi} — terakhir {Number(v.kilometer_terakhir || 0).toLocaleString('id-ID')} km</option>)}</select></label><label>Tanggal<input type="date" value={kmForm.tanggal} onChange={e => setKmForm(f => ({ ...f, tanggal: e.target.value }))} /></label><label>KM<input type="number" min="0" value={kmForm.kilometer} onChange={e => setKmForm(f => ({ ...f, kilometer: e.target.value }))} /></label><label>Sumber<select value={kmForm.sumber} onChange={e => setKmForm(f => ({ ...f, sumber: e.target.value }))}><option>SERVICE</option><option>PERMINTAAN</option><option>PEMERIKSAAN</option><option>MANUAL</option><option>LAINNYA</option></select></label><label className="full">Keterangan<input value={kmForm.keterangan} onChange={e => setKmForm(f => ({ ...f, keterangan: e.target.value }))} /></label><div className="full x-actions"><button type="button" className="x-btn secondary" onClick={() => { setEditingKmId(null); setKmForm(emptyKm) }}>Bersihkan</button><button className="x-btn primary" disabled={saving}>{editingKmId ? 'Perbarui KM' : 'Simpan KM'}</button></div></form></>}</section><section className="x-card"><div className="x-table-wrap"><table className="x-table"><thead><tr><th>Kendaraan</th><th>Tanggal</th><th>KM</th><th>Sumber</th><th>Keterangan</th><th>Aksi</th></tr></thead><tbody>{kms.map(k => <tr key={k.id}><td>{vehicleMap[k.kendaraan_id]?.nomor_polisi || '-'}</td><td>{dateText(k.tanggal)}</td><td>{Number(k.kilometer || 0).toLocaleString('id-ID')} km</td><td>{k.sumber || '-'}</td><td>{k.keterangan || '-'}</td><td className="x-action-compact"><button className="x-link" onClick={() => setSelectedKm(k)}>Detail</button>{canProcess && <button className="x-link" onClick={() => editKm(k)}>Edit</button>}{canProcess && <button className="x-link danger" onClick={() => deleteKm(k)} disabled={saving}>Hapus</button>}</td></tr>)}</tbody></table></div></section></>}
    {selected && <div className="x-overlay"><section className="x-modal x-modal-wide"><div className="x-modal-head"><div><span className="eyebrow">DETAIL SERVICE</span><h3>{selected.nomor_service || '#' + selected.id}</h3><small>{vehicleMap[selected.kendaraan_id]?.nomor_polisi || '-'} • {vehicleMap[selected.kendaraan_id]?.merk || '-'} {vehicleMap[selected.kendaraan_id]?.tipe || ''}</small></div><button onClick={() => setSelected(null)}>×</button></div>
      <div className="x-detail x-detail-grid">
        <p><b>Tanggal:</b> {dateText(selected.tanggal_service)}</p><p><b>KM:</b> {Number(selected.kilometer || 0).toLocaleString('id-ID')} km</p><p><b>Bengkel:</b> {selected.bengkel || '-'}</p><p><b>Jenis:</b> {TYPES[selected.jenis_service] || selected.jenis_service || '-'}</p>
        <p><b>Status:</b> {STAT[selected.status] || selected.status}</p><p><b>Approval:</b> {approvedCount(selected)} kali{additionalApprovalRequired(selected) ? ' — tambahan diperlukan' : ''}</p>
        <p><b>Estimasi:</b> {money(selected.estimasi_biaya)}</p><p><b>Biaya Aktual:</b> {money(selected.biaya_aktual)}</p><p><b>Total:</b> {money(selected.total ?? selected.biaya_aktual ?? selected.estimasi_biaya)}</p><p><b>Bukti:</b> {proofs.filter(p => p.service_id === selected.id).length} file</p>
        <div className="full"><b>Keluhan / Uraian:</b><p>{selected.keluhan || '-'}</p></div>
        <div className="full"><b>Rincian Item Service</b><div className="x-table-wrap detail-inner-table"><table className="x-table"><thead><tr><th>Item</th><th>Kategori</th><th>Qty</th><th>Harga</th><th>Subtotal</th></tr></thead><tbody>{items.filter(item => item.service_id === selected.id).length ? items.filter(item => item.service_id === selected.id).map(item => <tr key={item.id}><td>{item.nama_item}</td><td>{item.kategori || '-'}</td><td>{item.jumlah} {item.satuan}</td><td>{money(item.harga_satuan)}</td><td>{money(item.subtotal)}</td></tr>) : <tr><td colSpan="5">Belum ada item service.</td></tr>}</tbody></table></div></div>
      </div>
      <div className="x-actions"><button className="x-btn secondary" onClick={() => setSelected(null)}>Tutup</button>{canProcess && <button className="x-btn secondary" onClick={() => openServiceEdit(selected)}>Edit Service</button>}{canProcess && ['DISETUJUI', 'DALAM_PENGERJAAN'].includes(selected.status) && <><label className="x-inline-field">Biaya Aktual<input type="number" min="0" value={detailEdit.biaya_aktual} onChange={e => setDetailEdit({ biaya_aktual: e.target.value })} /></label><button className="x-btn secondary" onClick={updateActual} disabled={saving}>Simpan Biaya</button>{proofs.some(proof => proof.service_id === selected.id) ? <button className="x-btn primary" onClick={() => finish({ ...selected, biaya_aktual: Number(detailEdit.biaya_aktual || 0) })} disabled={saving}>Selesaikan Service</button> : <span className="x-note">Upload minimal satu bukti service sebelum menyelesaikan.</span>}</>}</div>
    </section></div>}
    {selectedItem && <div className="x-overlay"><section className="x-modal"><div className="x-modal-head"><div><span className="eyebrow">DETAIL ITEM</span><h3>{selectedItem.nama_item}</h3></div><button onClick={() => setSelectedItem(null)}>×</button></div><div className="x-detail"><p><b>Service:</b> #{selectedItem.service_id}</p><p><b>Kategori:</b> {selectedItem.kategori || '-'}</p><p><b>Jumlah:</b> {selectedItem.jumlah} {selectedItem.satuan || ''}</p><p><b>Harga Satuan:</b> {money(selectedItem.harga_satuan)}</p><p><b>Subtotal:</b> {money(selectedItem.subtotal)}</p><p><b>Keterangan:</b> {selectedItem.keterangan || '-'}</p></div><div className="x-actions"><button className="x-btn secondary" onClick={() => setSelectedItem(null)}>Tutup</button></div></section></div>}
    {selectedPart && <div className="x-overlay"><section className="x-modal"><div className="x-modal-head"><div><span className="eyebrow">DETAIL PENGGANTIAN</span><h3>{tab === 'ban' ? 'Ban' : 'Aki / Baterai'}</h3></div><button onClick={() => setSelectedPart(null)}>×</button></div><div className="x-detail"><p><b>Kendaraan:</b> {vehicleMap[selectedPart.kendaraan_id]?.nomor_polisi || '-'}</p><p><b>Tanggal:</b> {dateText(selectedPart.tanggal_penggantian)}</p><p><b>KM:</b> {Number(selectedPart.kilometer || 0).toLocaleString('id-ID')} km</p><p><b>Alasan:</b> {selectedPart.alasan_penggantian || '-'}</p><p><b>Biaya:</b> {money(selectedPart.biaya)}</p><p><b>Keterangan:</b> {selectedPart.keterangan || '-'}</p></div><div className="x-actions"><button className="x-btn secondary" onClick={() => setSelectedPart(null)}>Tutup</button><button className="x-btn primary" onClick={() => { setSelectedPart(null); editPart(selectedPart) }}>Edit</button></div></section></div>}
    {selectedKm && <div className="x-overlay"><section className="x-modal"><div className="x-modal-head"><div><span className="eyebrow">DETAIL KILOMETER</span><h3>{vehicleMap[selectedKm.kendaraan_id]?.nomor_polisi || '-'}</h3></div><button onClick={() => setSelectedKm(null)}>×</button></div><div className="x-detail"><p><b>Tanggal:</b> {dateText(selectedKm.tanggal)}</p><p><b>KM:</b> {Number(selectedKm.kilometer || 0).toLocaleString('id-ID')} km</p><p><b>Sumber:</b> {selectedKm.sumber || '-'}</p><p><b>Keterangan:</b> {selectedKm.keterangan || '-'}</p></div><div className="x-actions"><button className="x-btn secondary" onClick={() => setSelectedKm(null)}>Tutup</button><button className="x-btn primary" onClick={() => { setSelectedKm(null); editKm(selectedKm) }}>Edit</button></div></section></div>}
    {approvalModal && <div className="x-overlay"><section className="x-modal"><div className="x-modal-head"><div><span className="eyebrow">APPROVAL SERVICE</span><h3>{approvalModal.service.nomor_service || `#${approvalModal.service.id}`}</h3></div><button onClick={() => setApprovalModal(null)}>×</button></div><div className="x-detail"><p><b>Nilai yang dinilai:</b> {money(Number(approvalModal.service.biaya_aktual ?? approvalModal.service.estimasi_biaya ?? 0))}</p><p><b>Approval ke:</b> {approvalList(approvalModal.service).length + 1}</p><p><b>Pemberi approval:</b> {approvalModal.required === 'DIREKTUR' ? 'Direktur' : 'Atasan Transport'}</p><label>Catatan Approval<textarea value={approvalNote} onChange={e => setApprovalNote(e.target.value)} placeholder="Opsional" /></label><div className="x-actions"><button className="x-btn secondary" onClick={() => setApprovalModal(null)}>Batal</button><button className="x-btn danger" onClick={() => confirmApproval(false)} disabled={saving}>Tolak</button><button className="x-btn primary" onClick={() => confirmApproval(true)} disabled={saving}>Setujui</button></div></div></section></div>}
  </div>
}