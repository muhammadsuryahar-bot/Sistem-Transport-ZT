...
export default function ServiceFeaturePage({ profile }) {
  ...
  return <div className="x-page"><Header eyebrow="SERVICE & MAINTENANCE" title="Service & Perbaikan" text="Kelola pekerjaan, approval, rincian item, bukti, ban, aki, dan kilometer." action={<button className="x-btn secondary" onClick={load}>↻ Refresh</button>} />{error && <Alert type="error">{error}</Alert>}{success && <Alert>{success}</Alert>}<div className="x-tabs">{tabs.map(([v, l]) => <button key={v} className={tab === v ? 'active' : ''} onClick={() => { clearMessages(); setTab(v) }}>{l}</button>)}</div>{tab === 'excel' && <section className={`x-card service-excel-card ${serviceExcelFullscreen ? 'service-excel-card-fullscreen' : ''}`}>
    ...
    {selected && <div className="x-overlay"><section className="x-modal"><div className="x-modal-head"><div><span className="eyebrow">DETAIL SERVICE</span><h3>{selected.nomor_service || `#${selected.id}`}</h3></div><button onClick={() => setSelected(null)}>×</button></div><div className="x-detail"><p><b>Kendaraan:</b> {vehicleMap[selected.kendaraan_id]?.nomor_polisi || '-'}</p><p><b>Jenis:</b> {TYPES[selected.jenis_service] || selected.jenis_service}</p><p><b>Estimasi:</b> {money(selected.estimasi_biaya)}</p><p><b>Status:</b> {STAT[selected.status] || selected.status}</p><p><b>Keluhan:</b> {selected.keluhan || '-'}</p><p><b>Approval:</b> {approvedCount(selected)} kali{additionalApprovalRequired(selected) ? ' — tambahan diperlukan' : ''}</p>{canProcess && ['DISETUJUI', 'DALAM_PENGERJAAN'].includes(selected.status) && <><label>Biaya Aktual<input type="number" min="0" value={detailEdit.biaya_aktual} onChange={e => setDetailEdit({ biaya_aktual: e.target.value })} /></label><div className="x-actions"><button className="x-btn secondary" onClick={updateActual} disabled={saving}>Simpan Biaya Aktual</button>{proofs.some(proof => proof.service_id === selected.id) ? <button className="x-btn primary" onClick={() => finish({ ...selected, biaya_aktual: Number(detailEdit.biaya_aktual || 0) })} disabled={saving}>Selesaikan Service</button> : <span className="x-note">Upload minimal satu bukti service sebelum menyelesaikan.</span>}</div></>}</div></section></div>}
