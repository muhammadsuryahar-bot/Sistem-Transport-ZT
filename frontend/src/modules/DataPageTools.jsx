...
import VehicleExcelImportModalV2 from './VehicleExcelImportModalV2.jsx'
...
const ROW_MARKS = {
  NONE: { label: 'Belum ditandai', className: '' }, TODO: { label: 'Perlu dikerjakan', className: 'dpt-row-mark-todo' },
  PROCESS: { label: 'Sedang dikerjakan', className: 'dpt-row-mark-process' }, DONE: { label: 'Sudah selesai', className: 'dpt-row-mark-done' }, CHECKED: { label: 'Sudah dicek', className: 'dpt-row-mark-checked' },
}
const ROW_MARK_STORAGE = 'transport_excel_row_marks_v2'
const hasReport = value => value && typeof value === 'object' && value.context

function readRowMarks() { try { const raw = localStorage.getItem(ROW_MARK_STORAGE); const value = raw ? JSON.parse(raw) : {}; return value && typeof value === 'object' ? value : {} } catch { return {} } }
