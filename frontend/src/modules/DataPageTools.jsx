...
function likelyContext(name, rows, wanted) {
  ...
  const score = {
    kendaraan: ['nomor_polisi','no_polisi','merk','type','tipe'].filter(x => h.includes(x)).length,
    service: ['jenis_pekerjaan','uraian','qty','sat','harga_satuan','nilai_dpp'].filter(x => h.includes(x)).length,
    dokumen: ['nomor_polisi','stnk','kir','5_tahun'].filter(x => h.includes(x)).length,
...
async function importKendaraan(sheet, profile) {
  ...
  for (const row of rows) {
    const plate = valueOf(row, headers, 'nomor_polisi').toUpperCase(); if (!plate || seen.has(plate)) { skipped += 1; continue } seen.add(plate)
    const ownershipRaw = clean(valueOf(row, headers, 'pemilik')).toUpperCase()
    const payload = {
      kode_kendaraan: (valueOf(row, headers, 'kode_kendaraan') || `IMP-${plate.replace(/\W+/g,'')}`).toUpperCase(),
      nomor_polisi: plate, merk: valueOf(row, headers, 'merk'), tipe: valueOf(row, headers, 'tipe') || null,
      jenis_kendaraan: valueOf(row, headers, 'jenis_kendaraan') || null, tahun: numberValue(valueOf(row, headers, 'tahun')) || null,
      nomor_mesin: valueOf(row, headers, 'nomor_mesin') || null, nomor_rangka: valueOf(row, headers, 'nomor_rangka') || null,
      pemilik: valueOf(row, headers, 'pemilik') || null, lokasi: valueOf(row, headers, 'lokasi') || null,
      kilometer_terakhir: numberValue(valueOf(row, headers, 'kilometer')) ?? 0,
      kepemilikan: /\bSEWA\b|BPK\.|PRIBADI|PEMILIK/i.test(ownershipRaw) ? 'SEWA' : 'ASET_KANTOR',
      jenis_sewa: null, status: 'ACTIVE', keterangan: valueOf(row, headers, 'keterangan') || null,
    }
    const existing = await supabase.from('kendaraan').select('id').eq('nomor_polisi', plate).maybeSingle(); if (existing.error) throw existing.error
    if (existing.data?.id) { const r = await supabase.from('kendaraan').update(payload).eq('id', existing.data.id); if (r.error) throw r.error; updated += 1 }
    else { const r = await supabase.from('kendaraan').insert(payload); if (r.error) throw r.error; added += 1 }
  }
