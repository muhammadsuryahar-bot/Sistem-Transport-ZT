...
function normalizeImportPreviewTables() {
  ...
  tables.forEach((table) => {
    const headerCells = Array.from(table.querySelectorAll('thead th')).filter((cell) => !cell.classList.contains('dpt-row-mark-head'))
    ...
    table.querySelectorAll('tbody tr').forEach((row) => {
      ...
      cells.forEach((cell, index) => {
        ...
        if (!raw || cell.querySelector('input,select,textarea')) return
...
function getRowKey(context, row) {
  ...
  const values = cells.map((cell) => {
    ...
    return controls || cell.textContent.trim()
  })
  return `${context}:${simpleHash(values.join('\u241f') || `row-${row.rowIndex}`)}`
}
...
function ensureRowMarkControls(context) {
  ...
  tables.forEach((table) => {
    ...
    if (!toolbar) {
      ...
      toolbar.innerHTML = `<div class="dpt-row-mark-title"><b>Penanda kerja</b><span>Tandai status tiap baris agar pekerjaan Transport tidak perlu ditandai di Excel lagi.</span></div><div class="dpt-row-mark-controls"><label>Filter <select class="dpt-row-mark-filter"><option value="ALL">Semua</option><option value="NONE">Belum ditandai</option><option value="TODO">Perlu dikerjakan</option><option value="PROCESS">Sedang dikerjakan</option><option value="DONE">Sudah selesai</option><option value="CHECKED">Sudah dicek</option></select></label><button type="button" class="dpt-row-mark-clear">Hapus semua tanda</button></div>`
    ...
    if (filter && !filter.dataset.bound) {
      ...
      filter.addEventListener('change', () => {
        ...
        table.querySelectorAll('tbody tr').forEach((row) => {
          ...
          row.style.display = value === 'ALL' || (latestMarks[key] || 'NONE') === value ? '' : 'none'
        })
      })
    }
...
export default function DataPageTools({ context, profile, onExport }) {
  ...
  useEffect(() => {
    let timer = null
    const run = () => { normalizeImportPreviewTables(); if (showImport) ensureRowMarkControls(context) }
    run()
    const observer = new MutationObserver(() => { window.clearTimeout(timer); timer = window.setTimeout(run, 40) })
      ...
      : context === 'pengajuan'
        ...
        : context === 'service'
          ...
          : context === 'dokumen'
            ...
            : context === 'sewa'
              ? <RentalHistoryImportModalV2 profile={profile} onClose={closeImport} onDone={finishImport} />
              : <UnifiedExcelImportModalSafe context={context} profile={profile} onClose={closeImport} onDone={finishImport} />
    )}
    {importReport && <div className="dpt-overlay" role="dialog" aria-modal="true" aria-label="Laporan hasil import"><section className="dpt-modal import-result-modal"><header className="dpt-modal-head"><div><span className="eyebrow">IMPORT SELESAI</span><h3>Rekonsiliasi Data {CONTEXT_LABEL[importReport.context] || 'Excel'}</h3><p>Perbedaan jumlah antara Excel dan data sistem dijelaskan di sini supaya Admin dapat memastikan tidak ada data yang hilang tanpa alasan.</p></div><button type="button" className="dpt-icon" onClick={closeReport}>×</button></header><div className="service-import-stats"><div><b>{importReport.sourceRows ?? 0}</b><span>baris sumber</span></div><div><b>{importReport.validRows ?? importReport.sourceRows ?? 0}</b><span>baris valid</span></div><div><b>{importReport.uniqueVehicles ?? importReport.transactions ?? importReport.imported ?? 0}</b><span>data unik</span></div><div><b>{importReport.mergedDuplicates ?? importReport.skipped ?? 0}</b><span>duplikat/skip</span></div></div><div className="vehicle-import-explanation"><b>Hasil penyimpanan</b>{importReport.added != null && <span>Data baru: <strong>{importReport.added}</strong></span>}{importReport.updated != null && <span>Data diperbarui: <strong>{importReport.updated}</strong></span>}{importReport.driversCreated != null && <span>Driver dibuat: <strong>{importReport.driversCreated}</strong></span>}{importReport.items != null && <span>Item detail tersimpan: <strong>{importReport.items}</strong></span>}{importReport.kmUpdated != null && <span>KM kendaraan diperbarui: <strong>{importReport.kmUpdated}</strong></span>}{importReport.unknownPlates?.length > 0 && <span>Plat belum ada di Master Kendaraan: <strong>{importReport.unknownPlates.length}</strong></span>}</div>{importReport.message && <div className="vehicle-import-note"><b>Detail hasil import</b><span>{importReport.message}</span></div>}{importReport.context === 'kendaraan' && <div className="vehicle-import-note"><b>Catatan rekonsiliasi kendaraan</b><span>Excel menghitung baris sumber, sedangkan Master Kendaraan menggunakan No. Polisi sebagai identitas unik.</span><span>Baris dengan No. Polisi yang sama dapat diperbarui, bukan dibuat sebagai kendaraan baru kedua.</span></div>}{importReport.missingSourceNumbers?.length > 0 && <div className="vehicle-import-warning">Nomor urut sumber yang tidak ditemukan: <b>{importReport.missingSourceNumbers.join(', ')}</b>.</div>}{importReport.unknownPlates?.length > 0 && <div className="vehicle-import-warning">Plat belum ada di Master Kendaraan: <b>{importReport.unknownPlates.slice(0, 20).join(', ')}</b>{importReport.unknownPlates.length > 20 ? ' …' : ''}</div>}<div className="dpt-actions"><button type="button" className="dpt-button" onClick={closeReport}>Tutup</button><button type="button" className="dpt-button primary" onClick={refreshAfterImport}>Refresh Data Sistem</button></div></section></div>}
