...
const ALIASES = {
  ...
  keluhan: ['keluhan', 'keluhan_kerusakan', 'uraian_kerusakan'],
  prioritas: ['prioritas'],
...
const BLOCKED_SHEETS = {
  pengajuan: new Set(['permintaan_perbaikan', 'pengajuan_perbaikan', 'data_service', 'data_kendaraan', 'data_genset', 'stnk', 'stnk_dan_kir']),
...
async function parseXlsx(file) {
  ...
  const relMap = Object.fromEntries(Array.from(rels.querySelectorAll('Relationship')).map((r) => [r.getAttribute('Id'), r.getAttribute('Target')])); const sheets = []
...
function findHeader(rows) { let best = { index: -1, row: [], score: -1 }; rows.slice(0, 40).forEach((row, idx) => { const score = row.filter(Boolean).map(norm).filter(Boolean).length; if (score > best.score) best = { index: idx, row, score } }); return best.index >= 0 ? best : { index: 0, row: rows[0] || [], score: 0 } }
function sheetInfo(sheet) { const header = findHeader(sheet.rows); return { ...sheet, headerIndex: header.index, headers: header.row } }
...
function excelDate(value) { const v = clean(value); if (!v) return null; if (/^\d{4}-\d{2}-\d{2}$/.test(v)) return v; if (/^\d{1,2}[/-]\d{1,2}[/-]\d{4}$/.test(v)) { const [d, m, y] = v.split(/[/-]/); return `${y}-${String(m).padStart(2, '0')}-${String(d).padStart(2, '0')}` }; const serial = Number(v); if (Number.isFinite(serial) && serial > 20000 && serial < 80000) return new Date(Date.UTC(1899, 11, 30) + serial * 86400000).toISOString().slice(0, 10); const date = new Date(v); return Number.isNaN(date.getTime()) ? null : date.toISOString().slice(0, 10) }
...
function requiredKeys(context) { return context === 'pengajuan' ? ['nomor_polisi', 'tanggal', 'keluhan'] : ['nomor_kontrak', 'nomor_polisi', 'pemilik', 'tanggal_mulai', 'tanggal_selesai', 'nilai_sewa_bulanan'] }
function validateSource(sheet, context) {
  const name = norm(sheet.name)
  if (BLOCKED_SHEETS[context]?.has(name)) throw new Error(context === 'pengajuan' ? `Sheet “${sheet.name}” bukan tabel Pengajuan Service. Pilih Excel yang memiliki header Nomor Polisi + Tanggal + Keluhan. Data Service tidak boleh diimpor sebagai pengajuan.` : `Sheet “${sheet.name}” bukan tabel Kontrak Sewa. Gunakan sheet kontrak yang memiliki Nomor Kontrak + kendaraan + periode sewa.`)
  const missing = requiredKeys(context).filter((key) => !hasHeader(sheet.headers, key))
  if (missing.length) { const names = { nomor_polisi: 'Nomor Polisi', tanggal: 'Tanggal', keluhan: 'Keluhan', nomor_kontrak: 'Nomor Kontrak', pemilik: 'Pemilik', tanggal_mulai: 'Tanggal Mulai', tanggal_selesai: 'Tanggal Selesai', nilai_sewa_bulanan: 'Nilai Sewa Bulanan' }; throw new Error(`Format Excel ${LABELS[context]} tidak sesuai. Kolom wajib belum ada: ${missing.map((key) => names[key]).join(', ')}.`) }
  const rows = dataRows(sheet); if (!rows.length) throw new Error('Sheet yang dipilih tidak memiliki baris data.'); return rows
}
...
export default function UnifiedExcelImportModalSafe({ context, profile, onDone, onClose }) {
  ...
  const scan = async (nextFile) => {
    setFile(nextFile || null); setSelected(null); setError(''); setMessage(''); if (!nextFile) return; if (!/\.xlsx$/i.test(nextFile.name)) return setError('Gunakan file Excel .xlsx. Format .xls lama belum didukung.'); if (nextFile.size > MAX_FILE_SIZE) return setError('Ukuran file maksimal 25 MB.')
    setLoading(true)
    try {
      const sheets = (await parseXlsx(nextFile)).map(sheetInfo).map((sheet) => ({ ...sheet, score: scoreSheet(sheet, context) })).sort((a, b) => b.score - a.score); const candidate = sheets[0]
      if (!candidate || candidate.score < 100) { const name = candidate?.name ? ` Sheet teratas: “${candidate.name}”.` : ''; throw new Error(`Format Excel ${LABELS[context]} belum cocok dengan template sistem.${name} Template wajib: ${context === 'pengajuan' ? 'Nomor Polisi, Tanggal, Keluhan' : 'Nomor Kontrak, Nomor Polisi, Pemilik, Tanggal Mulai, Tanggal Selesai, Nilai Sewa Bulanan'}.`) }
      validateSource(candidate, context); setSelected(candidate); setMessage(`Sheet “${candidate.name}” valid • ${dataRows(candidate).length} baris data.`)
