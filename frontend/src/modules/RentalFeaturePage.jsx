...
export default function RentalFeaturePage({ profile }) {
  ...
  return <div className="x-page">
    ...
    {tab === 'pemilik' && <section className="x-card">
      <div className="x-card-title"><div><h3>{editingOwnerId ? 'Edit Pemilik Sewa' : 'Data Pemilik Sewa'}</h3><p>Data pemilik digunakan oleh kontrak rental. Identitas kendaraan tetap dikelola di Master Kendaraan.</p></div>{editingOwnerId && <button className="x-btn secondary" type="button" onClick={resetOwnerForm}>Batal Edit</button>}</div>
    ...
    {tab === 'kontrak' && <section className="x-card">
      <div className="x-card-title"><div><h3>{editingContractId ? 'Edit Kontrak Sewa — 6 Bulan' : 'Kontrak Sewa Baru — 6 Bulan'}</h3><p>Nomor rangka tidak diperlukan di modul administrasi sewa; kendaraan dipilih dari nomor polisi.</p></div>{editingContractId&&<button className="x-btn secondary" type="button" onClick={resetContractForm}>Batal Edit</button>}</div>
    ...
    {tab === 'pembayaran' && <section className="x-card">
      <div className="x-card-title"><div><h3>{editingPaymentId ? 'Edit Pembayaran Sewa Bulanan' : 'Pembayaran Sewa Bulanan'}</h3><p>Rekap pembayaran rental per kontrak. No. rangka tidak digunakan di administrasi sewa.</p></div>{editingPaymentId&&<button className="x-btn secondary" type="button" onClick={resetPaymentForm}>Batal Edit</button>}</div>
    ...
    {tab === 'repair' && <section className="x-card">
      <div className="x-card-title"><div><h3>{editingRepairId ? 'Edit Perbaikan Kendaraan Sewa' : 'Perbaikan Kendaraan Sewa'}</h3><p>Catat pembayaran kantor dan bukti perbaikan. Setelah dibayar kantor, biaya dapat ditandai untuk dipotong dari rental.</p></div>{editingRepairId&&<button className="x-btn secondary" type="button" onClick={resetRepairForm}>Batal Edit</button>}</div>
    ...
    {contractDetail && <div className="x-overlay"><section className="x-modal"><div className="x-modal-head"><div><span className="eyebrow">DETAIL KONTRAK SEWA</span><h3>{contractDetail.nomor_kontrak || '#' + contractDetail.id}</h3></div><button onClick={()=>setContractDetail(null)}>×</button></div><div className="x-detail"><p><b>No. Polisi:</b> {vehicleMap[contractDetail.kendaraan_id]?.nomor_polisi || '-'}</p><p><b>Pemilik:</b> {owners.find(o=>o.id===contractDetail.pemilik_sewa_id)?.nama_pemilik || '-'}</p><p><b>Periode:</b> {dateText(contractDetail.tanggal_mulai)} s/d {dateText(contractDetail.tanggal_selesai)} ({contractDetail.periode_bulan} bulan)</p><p><b>Nilai Sewa:</b> {money(contractDetail.nilai_sewa_bulanan)} / bulan</p><p><b>Jatuh Tempo Bulanan:</b> Tanggal {contractDetail.tanggal_jatuh_tempo_bulanan || '-'}</p><p><b>Status:</b> {contractDetail.status}</p><p><b>Catatan:</b> {contractDetail.catatan || '-'}</p></div><div className="x-actions"><button className="x-btn secondary" onClick={()=>setContractDetail(null)}>Tutup</button>{contractDetail.dokumen_kontrak_path&&<button className="x-btn secondary" onClick={()=>openFile('contract-detail-'+contractDetail.id,'dokumen-sewa',contractDetail.dokumen_kontrak_path)}>Lihat Dokumen</button>}{editable&&<button className="x-btn primary" onClick={()=>{setContractDetail(null);editContract(contractDetail)}}>Edit</button>}</div></section></div>}
    {repairDetail && <div className="x-overlay"><section className="x-modal"><div className="x-modal-head"><div><span className="eyebrow">DETAIL PERBAIKAN RENTAL</span><h3>{repairDetail.nomor_perbaikan || '#' + repairDetail.id}</h3></div><button onClick={()=>setRepairDetail(null)}>×</button></div><div className="x-detail"><p><b>No. Polisi:</b> {vehicleMap[repairDetail.kendaraan_id]?.nomor_polisi || '-'}</p><p><b>Tanggal:</b> {dateText(repairDetail.tanggal_kejadian)}</p><p><b>KM:</b> {repairDetail.kilometer || '-'}</p><p><b>Kerusakan:</b> {repairDetail.jenis_kerusakan || '-'}</p><p><b>Penyebab:</b> {repairDetail.penyebab || '-'}</p><p><b>Estimasi:</b> {money(repairDetail.estimasi_biaya)}</p><p><b>Aktual:</b> {money(repairDetail.biaya_aktual)}</p><p><b>Dibayar Kantor:</b> {repairDetail.dibayar_kantor ? 'Ya' : 'Tidak'}</p><p><b>Dapat Dipotong:</b> {repairDetail.dapat_dipotong ? money(repairDetail.jumlah_dipotong) : 'Tidak'}</p><p><b>Status:</b> {repairDetail.status}</p><p><b>Deskripsi:</b> {repairDetail.deskripsi_kerusakan || '-'}</p><p><b>Catatan:</b> {repairDetail.catatan || '-'}</p></div><div className="x-actions"><button className="x-btn secondary" onClick={()=>setRepairDetail(null)}>Tutup</button>{repairEditable&&<button className="x-btn primary" onClick={()=>{setRepairDetail(null);editRepair(repairDetail)}}>Edit</button>}</div></section></div>}
