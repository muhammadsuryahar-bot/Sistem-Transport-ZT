...
function findHeader(sheet) {
  ...
  sheet.rows.slice(0, 80).forEach((item, idx) => {
    const score = item.values.filter(value => clean(value)).map(norm).filter(Boolean).length
...
function repairRow(row, headers) {
  const out = {
    ...
    ownership: upper(valueOf(row, headers, 'status')),
    masa_pajak_raw: valueOf(row, headers, 'masa_pajak'),
    status_pajak: upper(valueOf(row, headers, 'status_pajak')),
  ...
  if (out.masa_pajak_raw && !excelDate(out.masa_pajak_raw)) {
    if (!out.unit_kerja) out.unit_kerja = out.masa_pajak_raw
    out.masa_pajak_raw = ''
  }
  if (out.status_pajak && !['HIDUP', 'MATI'].includes(out.status_pajak)) {
    if (!out.driver) out.driver = out.status_pajak
    out.status_pajak = ''
  }
  out.masa_pajak = excelDate(out.masa_pajak_raw)
  if (['ASET', 'MILIK', 'MILIK KANTOR', 'ASET KANTOR'].includes(out.ownership)) out.kepemilikan = 'ASET_KANTOR'
...
function mergeRows(group) {
  ...
  return { ...base, sourceRows: group.map(item => item.excelRow) }
}
...
export default function VehicleExcelImportModalV2({ profile, onDone, onClose }) {
  ...
  const scan = async nextFile => {
    ...
    try {
      ...
      const chosen = sheets.find(sheet => norm(sheet.name) === 'data_kendaraan') || sheets.find(sheet => /data\s*kendaraan/i.test(sheet.name)) || sheets.find(sheet => { const headers = findHeader(sheet).row.map(norm); return headers.includes('no_pol') && headers.includes('merk') })
      if (!chosen) throw new Error('Sheet Data Kendaraan tidak ditemukan.')
      const header = findHeader(chosen)
      const data = chosen.rows.slice(header.index + 1).filter(row => row.values.some(value => clean(value)))
      ...
      setWorkbook({ sheet: chosen, header, data: repaired, valid, uniqueCount: groups.size, duplicateCount: valid.length - groups.size, missingSourceNumbers })
      setMessage(`Pemeriksaan selesai: ${repaired.length} baris sumber dibaca • ${valid.length} valid • ${groups.size} No. Polisi unik.`)
  ...
  const start = async () => {
    ...
    try {
      const result = await importVehicleRows(workbook.valid, profile)
  ...
  return <div className="dpt-overlay" role="dialog" aria-modal="true" aria-label="Import Excel Kendaraan">
    <section className="dpt-modal vehicle-import-modal">
      <header className="dpt-modal-head"><div><span className="eyebrow">IMPORT EXCEL KENDARAAN</span><h3>Data Kendaraan</h3><p>Preview membaca seluruh baris dari sheet Data Kendaraan. Tidak ada lagi batas 8 baris.</p></div><button type="button" className="dpt-icon" onClick={onClose}>×</button></header>
      ...
      {selected && <>
        ...
        <div className="vehicle-import-explanation"><b>Rekonsiliasi</b><span>Semua baris Excel tetap tampil di preview. Saat penyimpanan, No. Polisi dipakai sebagai identitas unik agar kendaraan yang sama tidak dibuat dua kali.</span><span>{workbook.data.length} baris sumber → {workbook.valid.length} valid → {workbook.uniqueCount} kendaraan unik → {workbook.duplicateCount} duplikat digabung.</span></div>
