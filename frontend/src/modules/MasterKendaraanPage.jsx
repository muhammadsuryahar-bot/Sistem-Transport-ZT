...
export default function MasterKendaraanPage({ profile }) {
  ...
  const loadData = async () => {
    ...
    const [v, d] = await Promise.all([
      supabase.from('kendaraan').select('id,kode_kendaraan,nomor_polisi,merk,tipe,jenis_kendaraan,tahun,warna,nomor_rangka,nomor_mesin,status_pajak,unit_kerja,catatan_hutang,kepemilikan,jenis_sewa,pemilik,driver_id,lokasi,kilometer_terakhir,status,kondisi,keterangan,created_at,updated_at').order('nomor_polisi'),
  ...
  const filtered = useMemo(() => {
    ...
    return vehicles.filter((v) => {
      ...
      const text = [v.kode_kendaraan,v.nomor_polisi,v.merk,v.tipe,v.jenis_kendaraan,v.pemilik,v.lokasi,v.unit_kerja,v.status_pajak,v.kondisi,driver?.nama_lengkap].filter(Boolean).join(' ').toLowerCase()
  ...
  const deleteDriver = async (driver) => {
    if (!canDeleteDriver) { setError('Hanya Administrator yang dapat menghapus driver. Nonaktifkan driver bila sudah tidak digunakan.'); return }
  ...
  return <div className="master-page">
    <div className="master-head"><div><span className="eyebrow">MASTER DATA</span><h2>Kendaraan & Driver</h2><p>Kelola pusat data armada, kepemilikan, driver/PIC, lokasi, dan kilometer.</p></div><div className="master-actions">{canEditMaster && <button className="m-btn secondary" onClick={() => openDriver()}>+ Driver</button>}{canEditMaster && <button className="m-btn primary" onClick={() => openVehicle()}>+ Kendaraan</button>}</div></div>
