  ...
  const exportCurrentPage = async () => {
    ...
    try {
      ...
      if (activePage === 'kendaraan') {
        ...
        const rentalVehicleRows = (rs[5].data || []).map((v, index) => { const c = (rs[1].data || []).find(row => Number(row.kendaraan_id) === Number(v.id) && row.status === 'AKTIF') || (rs[1].data || []).find(row => Number(row.kendaraan_id) === Number(v.id)); const o = ownerMap[c?.pemilik_sewa_id]; return { no: index + 1, nomor_polisi: v.nomor_polisi || '-', merk: v.merk || '-', tipe: v.tipe || '-', jenis_sewa: v.jenis_sewa === 'SEWA_PERORANGAN' ? 'Sewa Perorangan' : v.jenis_sewa === 'SEWA_RENTAL' ? 'Sewa Rental' : 'Belum ditentukan', pemilik: o?.nama_pemilik || v.pemilik || '-', perusahaan: o?.nama_perusahaan || '-', lokasi: v.lokasi || '-', unit_kerja: v.unit_kerja || '-', nomor_kontrak: c?.nomor_kontrak || '-', tanggal_mulai: c?.tanggal_mulai || '-', tanggal_selesai: c?.tanggal_selesai || '-', nilai_sewa_bulanan: c?.nilai_sewa_bulanan ?? '-', status_kontrak: c?.status || 'BELUM ADA KONTRAK' } })
