...
const STATS = [
  ['vehicles', 'Total Kendaraan', 'Data kendaraan terdaftar', '▣'],
  ['activeVehicles', 'Kendaraan Aktif', 'Siap digunakan', '✓'],
  ['inService', 'Sedang Service', 'Perlu dipantau', '⚙'],
  ['pendingRequests', 'Pengajuan Menunggu', 'Perlu diproses', '↗'],
]
...
function AppTransport() {
  ...
  useEffect(() => {
    if (session && profile && !allowedPages.includes(activePage)) setActivePage('dashboard')
  }, [activePage, allowedPages, profile, session])
  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      ...
      if (!cancelled) {
        setStats({ vehicles: vehicles.count || 0, activeVehicles: activeVehicles.count || 0, inService: inService.count || 0, pendingRequests: pendingRequests.count || 0 })
  ...
  const handleLogin = async (event) => {
    ...
    if (!cleanEmail || !password) { setErrorMessage('Email dan password wajib diisi.'); return }
    setSubmitting(true)
    const { data, error } = await supabase.auth.signInWithPassword({ email: cleanEmail, password })
    if (error || !data.user) { setErrorMessage('Email atau password tidak sesuai.'); setSubmitting(false); return }
  ...
  if (loading) return <main className="auth-shell"><div className="loading-card"><div className="loading-spinner" aria-hidden="true"/><p>Memuat Sistem Transport...</p></div></main>

  if (!session || !profile) return (
    <main className="auth-shell">
      <section className="login-card" aria-label="Form login Sistem Transport">
        <div className="brand-mark">ZT</div><span className="eyebrow">PT ZAMAN TEKNINDO</span>
  ...
  return (
    <div className="dashboard-layout">
      ...
      <aside className={`sidebar ${sidebarOpen ? 'sidebar-open' : ''}`}>
        <div className="sidebar-brand"><div className="brand-mark small">ZT</div><div><strong>Zaman Teknindo</strong><span>Transport System</span></div></div>
        <div className="nav-section-label">MENU UTAMA</div>
        <nav className="sidebar-nav" aria-label="Navigasi utama">{visibleNavItems.map((item) => <button key={item.id} className={`nav-item ${activePage === item.id ? 'active' : ''}`} onClick={() => { setActivePage(item.id); setSidebarOpen(false) }}><span className="nav-icon">{item.icon}</span><span>{item.label}</span></button>)}</nav>
        <div className="sidebar-bottom"><div className="user-mini"><div className="avatar">{(profile.nama_lengkap || profile.email || 'U').charAt(0).toUpperCase()}</div><div className="user-mini-text"><strong>{profile.nama_lengkap || 'Pengguna'}</strong><span>{ROLE_LABELS[profile.role] || profile.role}</span></div></div><button className="logout-button" onClick={handleLogout} disabled={submitting}>Keluar</button></div>
      </aside>
      <main className="main-content">
        <header className="topbar"><button className="menu-button" onClick={() => setSidebarOpen(true)} aria-label="Buka menu">☰</button><div><span className="topbar-label">SISTEM TRANSPORT</span><h1>{NAV_ITEMS.find((item) => item.id === activePage)?.label || 'Dashboard'}</h1></div><div className="topbar-user"><div className="avatar">{(profile.nama_lengkap || profile.email || 'U').charAt(0).toUpperCase()}</div><div><strong>{profile.nama_lengkap || profile.email}</strong><span>{ROLE_LABELS[profile.role] || profile.role}</span></div></div></header>
...
function DashboardHome({ profile, stats, statsLoading, onNavigate }) {
  ...
  return <>
    <section className="page-intro"><div><span className="eyebrow">{today}</span><h2>Selamat datang, {profile.nama_lengkap || 'Pengguna'}</h2><p>Pantau kondisi kendaraan dan aktivitas transport dari satu tempat.</p></div></section>
    <section className="stats-grid" aria-label="Ringkasan transport">{STATS.map(([key,label,helper,icon]) => <article className="stat-card" key={key}><div className="stat-icon">{icon}</div><div><span>{label}</span><strong>{statsLoading ? '...' : stats[key]}</strong><small>{helper}</small></div></article>)}</section>
    <section className="dashboard-grid">
      <article className="panel quick-panel"><div className="panel-heading"><div><span className="eyebrow">AKSES CEPAT</span><h3>Menu yang sering digunakan</h3></div></div><div className="quick-grid"><button className="quick-action" onClick={() => onNavigate('pengajuan')}><span>↗</span><strong>Ajukan Service</strong><small>Buat permintaan untuk kendaraan</small></button><button className="quick-action" onClick={() => onNavigate('kendaraan')}><span>▣</span><strong>Data Kendaraan</strong><small>Lihat armada dan kilometer</small></button><button className="quick-action" onClick={() => onNavigate('service')}><span>⚙</span><strong>Proses Service</strong><small>Pantau pekerjaan dan biaya</small></button><button className="quick-action" onClick={() => onNavigate('dokumen')}><span>□</span><strong>Dokumen</strong><small>Kelola STNK, KIR, dan lainnya</small></button></div></article>
      <article className="panel flow-panel"><div className="panel-heading"><div><span className="eyebrow">ALUR UTAMA</span><h3>Proses transport</h3></div></div><div className="flow-list">{['Operasional','Permintaan Service','Transport','Approval','Service','Bukti','Selesai'].map((item, i) => <div className="flow-item" key={item}><span>{i+1}</span><strong>{item}</strong></div>)}</div></article>
    </section>
  </>
}

function ModulePlaceholder({ title, role }) {
  return <section className="module-placeholder"><span className="eyebrow">MODUL {title.toUpperCase()}</span><h2>{title}</h2><p>Modul ini disiapkan untuk role <strong>{ROLE_LABELS[role] || role}</strong>. Fitur transaksi akan dibangun mengikuti alur bisnis dan hak akses sistem.</p></section>
}
