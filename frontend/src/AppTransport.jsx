    ...
    try {
      ...
      if (activePage === 'kendaraan') {
        ...
        exportToExcel(`Rekap-Administrasi-Sewa-ZT-${stamp}.xls`, [
          ...
          { title: 'RENTAL HISTORIS', columns: columns([['no_excel','No'],['tahun','Tahun'],['supplier','Supplier'],['uraian','Uraian'],['periode_tagihan','Periode Tagihan'],['nilai_invoice','Nilai Invoice']]), rows: (rs[5].data || []).map((h) => ({ no_excel: Number(h.source_no) || h.excel_row, tahun: h.tahun || '-', supplier: h.supplier || '-', uraian: h.uraian || '-', periode_tagihan: h.periode_tagihan || '-', nilai_invoice: h.nilai_invoice ?? 0 })).sort((a,b)=>Number(a.no_excel||0)-Number(b.no_excel||0)) },,
