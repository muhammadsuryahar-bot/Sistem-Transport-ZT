    ...
    try {
      ...
      if (activePage === 'kendaraan') {
        ...
        exportToExcel(`Rekap-Kendaraan-ZT-${stamp}.xls`, [
          { title: 'KENDARAAN', columns: columns([['no_export','No'],['merk','Merk'],['tipe','Type'],['jenis_kendaraan','Jenis'],['tahun','Tahun'],['nomor_polisi','No. Pol'],['nomor_mesin','No. Mesin'],['nomor_rangka','No. Rangka'],['pemilik','Pemilik'],['kepemilikan','Kepemilikan'],['jenis_sewa','Jenis Sewa'],['foto_stnk','Foto STNK'],['masa_berlaku_pajak','Masa Berlaku Pajak'],['status_pajak','Status Pajak'],['unit_kerja','Unit Kerja'],['driver','Driver'],['lokasi','Lokasi Kerja'],['keterangan','Keterangan'],['catatan_hutang','Catatan Hutang']]), rows: cleanRows(v.data || []).map((x, index) => ({ ...x, no_export: index + 1, kepemilikan: x.kepemilikan === 'ASET' || x.kepemilikan === 'ASET_KANTOR' ? 'Aset' : x.kepemilikan === 'SEWA' || x.kepemilikan === 'RENTAL' || x.kepemilikan === 'KENDARAAN_SEWA' ? 'Sewa' : '-', jenis_sewa: x.kepemilikan === 'SEWA' ? (x.jenis_sewa === 'SEWA_PERORANGAN' ? 'Sewa Perorangan' : x.jenis_sewa === 'SEWA_PERUSAHAAN' || x.jenis_sewa === 'SEWA_RENTAL' ? 'Sewa Perusahaan' : 'Belum ditentukan') : '-', foto_stnk: x.foto_stnk_path ? 'ADA' : 'BELUM ADA', driver: driverMap[x.driver_id]?.nama_lengkap || '-' })) },
        ...
        sourceServiceRows.sort((a, b) => Number(a.no || 0) - Number(b.no || 0) || String(a.nomor_polisi || '').localeCompare(String(b.nomor_polisi || ''), 'id'))
        ...
        rs.forEach((r, i) => { if (r.error) throw new Error(`${names[i]}: ${r.error.message}`) })
        const vehicleMap = Object.fromEntries((rs[6].data || []).map(x => [x.id, x]))
        ...
        const rentalVehicleRows = (rs[6].data || []).map((v, index) => { const c = (rs[1].data || []).find(row => Number(row.kendaraan_id) === Number(v.id) && row.status === 'AKTIF') || (rs[1].data || []).find(row => Number(row.kendaraan_id) === Number(v.id)); const o = ownerMap[c?.pemilik_sewa_id]; return { no: index + 1, nomor_polisi: v.nomor_polisi || '-', merk: v.merk || '-', tipe: v.tipe || '-', jenis_sewa: v.jenis_sewa === 'SEWA_PERORANGAN' ? 'Sewa Perorangan' : v.jenis_sewa === 'SEWA_PERUSAHAAN' || v.jenis_sewa === 'SEWA_RENTAL' ? 'Sewa Perusahaan' : 'Belum ditentukan', pemilik: o?.nama_pemilik || v.pemilik || '-', perusahaan: o?.nama_perusahaan || '-', lokasi: v.lokasi || '-', unit_kerja: v.unit_kerja || '-', nomor_kontrak: c?.nomor_kontrak || '-', tanggal_mulai: c?.tanggal_mulai || '-', tanggal_selesai: c?.tanggal_selesai || '-', nilai_sewa_bulanan: c?.nilai_sewa_bulanan ?? '-', status_kontrak: c?.status || 'BELUM ADA KONTRAK' } })
        exportToExcel(`Rekap-Administrasi-Sewa-ZT-${stamp}.xls`, [
          { title: 'DAFTAR SEWA', columns: columns([['no','No'],['nomor_polisi','No. Polisi'],['merk','Merk'],['tipe','Type'],['jenis_sewa','Jenis Sewa'],['pemilik','Pemilik'],['perusahaan','Perusahaan/Rental'],['lokasi','Lokasi'],['unit_kerja','Unit Kerja'],['nomor_kontrak','No. Kontrak'],['tanggal_mulai','Tanggal Mulai'],['tanggal_selesai','Tanggal Selesai'],['nilai_sewa_bulanan','Sewa Bulanan'],['status_kontrak','Status Kontrak']]), rows: rentalVehicleRows },
          ...
          { title: 'RENTAL HISTORIS', columns: columns([['no_excel','No'],['tahun','Tahun'],['supplier','Supplier'],['uraian','Uraian'],['periode_tagihan','Periode Tagihan'],['nilai_invoice','Nilai Invoice']]), rows: (rs[5].data || []).map((h) => ({ no_excel: Number(h.source_no) || h.excel_row, excel_row: h.excel_row, tahun: h.tahun || '-', supplier: h.supplier || '-', uraian: h.uraian || '-', periode_tagihan: h.periode_tagihan || '-', nilai_invoice: h.nilai_invoice ?? 0 })).sort((a,b)=>Number(a.excel_row||0)-Number(b.excel_row||0)) },
