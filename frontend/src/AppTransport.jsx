  ...
  const exportCurrentPage = async () => {
    ...
    try {
      ...
      if (activePage === 'kendaraan') {
        ...
        exportToExcel(`Rekap-Kendaraan-Sewa-ZT-${stamp}.xls`, [
          { title: 'PEMILIK SEWA', columns: columns([['jenis_pemilik','Jenis Pemilik'],['nama_pemilik','Nama Pemilik'],['nomor_hp','Nomor HP'],['email','Email'],['alamat','Alamat'],['nomor_identitas','Nomor Identitas'],['nama_perusahaan','Nama Perusahaan'],['nomor_rekening','Nomor Rekening'],['nama_bank','Bank'],['aktif','Aktif'],['keterangan','Keterangan']]), rows: cleanRows(rs[0].data || []) },
          { title: 'KONTRAK SEWA', columns: columns([['nomor_kontrak','Nomor Kontrak'],['nomor_polisi','Nomor Polisi'],['pemilik','Pemilik'],['tanggal_mulai','Tanggal Mulai'],['tanggal_selesai','Tanggal Selesai'],['waktu_mulai','Waktu Mulai'],['waktu_selesai','Waktu Selesai'],['periode_bulan','Periode Bulan'],['nilai_sewa_bulanan','Nilai Sewa Bulanan'],['tanggal_jatuh_tempo_bulanan','Tanggal Jatuh Tempo'],['status','Status'],['catatan','Catatan']]), rows: cleanRows(rs[1].data || []).map(x => ({ ...x, nomor_polisi: vehicleMap[x.kendaraan_id]?.nomor_polisi || '-', pemilik: ownerMap[x.pemilik_sewa_id]?.nama_pemilik || '-' })) },
          { title: 'PEMBAYARAN SEWA', columns: columns([['nomor_kontrak','Nomor Kontrak'],['nomor_polisi','Nomor Polisi'],['periode_ke','Periode Ke'],['bulan_pembayaran','Bulan Pembayaran'],['tanggal_jatuh_tempo','Jatuh Tempo'],['jumlah_tagihan','Tagihan'],['jumlah_dibayar','Dibayar'],['status','Status'],['metode_pembayaran','Metode'],['nomor_referensi','No Referensi'],['bukti_pembayaran_path','Bukti'],['catatan','Catatan']]), rows: cleanRows(rs[2].data || []).map(x => ({ ...x, nomor_kontrak: contractMap[x.kontrak_sewa_id]?.nomor_kontrak || `#${x.kontrak_sewa_id}`, nomor_polisi: vehicleMap[contractMap[x.kontrak_sewa_id]?.kendaraan_id]?.nomor_polisi || '-' })) },
          { title: 'PERBAIKAN KENDARAAN SEWA', columns: columns([['nomor_polisi','Nomor Polisi'],['nomor_kontrak','Nomor Kontrak'],['tanggal_kejadian','Tanggal'],['kilometer','KM'],['jenis_kerusakan','Jenis Kerusakan'],['deskripsi_kerusakan','Deskripsi'],['penyebab','Penyebab'],['estimasi_biaya','Estimasi'],['biaya_aktual','Aktual'],['metode_penanganan','Metode'],['dibayar_kantor','Dibayar Kantor'],['tanggal_dibayar','Tanggal Dibayar'],['pemilik_diberitahu','Pemilik Diberitahu'],['status','Status'],['dapat_dipotong','Dapat Dipotong'],['jumlah_dipotong','Jumlah Dipotong'],['catatan','Catatan']]), rows: cleanRows(rs[3].data || []).map(x => ({ ...x, nomor_polisi: vehicleMap[x.kendaraan_id]?.nomor_polisi || '-', nomor_kontrak: contractMap[x.kontrak_sewa_id]?.nomor_kontrak || '-' })) },
          { title: 'POTONGAN RENTAL', columns: columns([['nomor_kontrak','Nomor Kontrak'],['perbaikan_sewa_id','ID Perbaikan'],['jumlah_potongan','Jumlah Potongan'],['catatan','Catatan'],['created_at','Dibuat']]), rows: cleanRows(rs[4].data || []).map(x => ({ ...x, nomor_kontrak: contractMap[rs[2].data?.find(p => p.id === x.pembayaran_sewa_id)?.kontrak_sewa_id]?.nomor_kontrak || '-' })) },
        ])
      } else if (activePage === 'dokumen') {
        const [{ data: docs, error: docError }, { data: vehicles, error: vehicleError }] = await Promise.all([
          supabase.from('dokumen_kendaraan').select('*').order('tanggal_jatuh_tempo'),
          supabase.from('kendaraan').select('id,nomor_polisi,merk,tipe').order('nomor_polisi'),
        ])
        if (docError) throw docError
        if (vehicleError) throw vehicleError
        const vehicleMap = Object.fromEntries((vehicles || []).map(x => [x.id, x]))
        exportToExcel(`Rekap-Dokumen-Kendaraan-ZT-${stamp}.xls`, [{ title: 'DOKUMEN KENDARAAN', columns: columns([['nomor_polisi','Nomor Polisi'],['jenis_dokumen','Jenis Dokumen'],['nomor_dokumen','Nomor Dokumen'],['tanggal_terbit','Tanggal Terbit'],['tanggal_berlaku_mulai','Berlaku Mulai'],['tanggal_jatuh_tempo','Jatuh Tempo'],['file_path','File Path'],['keterangan','Keterangan']]), rows: cleanRows(docs || []).map(x => ({ ...x, nomor_polisi: vehicleMap[x.kendaraan_id]?.nomor_polisi || '-' })) }])
      }
    } catch (error) {
      console.error('Page export error:', error)
      setErrorMessage(`Gagal mengekspor data halaman: ${error.message || 'kesalahan tidak diketahui'}`)
    } finally {
